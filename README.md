...
| `kavi chat -m "..."` | Single-turn [chat](docs/ARCHITECTURE.md#chat-v0): parse intent, plan, execute, return AgentResponse JSON. `--confirmed` pre-confirms side-effect skills |
...
├── agent/              # Chat v0: bounded conversational interface (parse → plan → execute)
...
└── decisions.md        # Append-only decision log (D001–D020)
```

## Development

```bash
uv run pytest -q              # Fast suite (~8s, 731 tests, no network)
