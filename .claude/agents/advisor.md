...
- **Pull plane** (early) — a conversational interface where the user talks to Kavi and it routes to trusted skills. The skill roster is the capability boundary.
- **Push plane** (not yet implemented) — the system observes patterns (what the user asks for, what fails, what's missing), proposes new skills, and enters them into the forge pipeline. Push can only propose. The human approves.
...
## Known Debt (acknowledged, do not re-flag unless worsened)
...
`spark.py` and `config.py` must NOT import from `kavi.forge`, `kavi.ledger`, or `kavi.policies`. Check:
...
- Scan for violations of the eight invariants
...
- Prioritize by: what unblocks the most, what tests the hardest invariant, what would be used daily
